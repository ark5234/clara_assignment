...
class DemoProcessor(BaseProcessor):
    ...
    def _build_v1_config(
        ...
        unknowns = self._build_unknowns(raw, client, business_hours, emergency_defs, integration)
    ...
    def _build_unknowns(
        ...
        def _add_if_missing(field: str, question: str, context: str | None = None) -> None:
            if field not in existing_fields:
                unknowns.append(
                    UnknownItem(field=field, question=question, context=context, priority="high", source_stage="demo")
                )
